...
FILELIST=/cluster/tufts/wongjiradlab/jmills09/mrcnn_processed_outs/mcc9_v29e_dl_run3b_bnb_intrinsic_nue_overlay_nocrtremerge/goodlist.txt
SUBMITNUM=0
ARRAYID=$SLURM_ARRAY_TASK_ID

# 2232 entries in goodlist for nue
for VARIABLE in {0..1}

do
  # echo $VARIABLE
  let line=(2 * $ARRAYID + $VARIABLE + 1)
  let line=($ARRAYID + 1950*$VARIABLE + 1)
