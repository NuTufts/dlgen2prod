...
#SBATCH --output=larmatchme_mcc9_run3_lowe_instrinsics.log
...
#SBATCH --error=/cluster/tufts/wongjiradlabnu/nutufts/dlgen2prod/larmatchme_gridlogs/griderr_larmatcheme_deploy_mcc9_run3_intrinsics_nue_sub0.%j.%N.err
...
FILEIDLIST=/cluster/tufts/wongjiradlab/nutufts/dlgen2prod/larmatch_scripts/larmatch_runlist_mcc9_v29e_dl_run3b_bnb_intrinsic_nue_LowE.txt
...
# CPU MODE
cd /cluster/tufts/
singularity exec ${container} bash -c "cd ${RUN_DLANA_DIR} && source run_batch_larmatchminkowski.sh $OFFSET $STRIDE $SAMPLE_NAME ${INPUTFILE} ${INPUTSTEM} ${FILEIDLIST}"
