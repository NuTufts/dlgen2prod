...
for ((i=0;i<${STRIDE};i++)); do
    ...
    CMD="python $LARMATCH_DIR/deploy_kps_larmatch.py --supera $baseinput --weights ${WEIGHTS_DIR}/${WEIGHT_FILE} --output $local_outfile --min-score 0.5 --adc-name wire --chstatus-name wire --device-name cuda --use-unet -tb"
