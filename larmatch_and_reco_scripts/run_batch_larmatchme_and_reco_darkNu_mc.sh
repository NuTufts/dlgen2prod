...
TAG=v3dev_lm_showerkp_retraining

# we assume we are already in the container
export OMP_NUM_THREADS=4

#WORKDIR=/cluster/tufts/wongjiradlabnu/nutufts/dlgen2prod/larmatch_and_reco_scripts/
#UBDL_DIR=/cluster/tufts/wongjiradlabnu/mrosen25/ubdl/
...
OUTPUT_DIR=/cluster/tufts/wongjiradlabnu/nutufts/data/${TAG}/${SAMPLE_NAME}/larflowreco/ana/
OUTPUT_LOGDIR=${WORKDIR}/logdir/${TAG}/${SAMPLE_NAME}
CONFIG_FILE=${WORKDIR}/config_larmatchme_deploycpu.yaml
COSMIC_DIR=/cluster/tufts/wongjiradlabnu/mrosen25/gen2val/scripts/repo
ADD_KPSRECO_RUNFLAGS="--save-all-keypoints"
...
for ((i=0;i<${STRIDE};i++)); do
    ...
    baseinputmod=$(echo $baseinput | sed 's|.root|_trueThrumu.root|g')
    echo "inputfile path: $inputfile" >> ${local_logfile}
    echo "baseinput: $baseinput" >> ${local_logfile}
    echo "baseinputmod: $baseinputmod" >> ${local_logfile}
    ...
    CMD="python3 $COSMIC_DIR/make_true_cosmic_image.py -i $baseinput"
    echo $CMD >> ${local_logfile}
    $CMD >> ${local_logfile} 2>&1

    #CMD="python3 $LARMATCH_DIR/deploy_larmatchme.py --config-file ${CONFIG_FILE} --supera $baseinputmod --weights ${WEIGHTS_DIR}/${WEIGHT_FILE} --output $lm_outfile --min-score 0.5 --adc-name wire --chstatus-name wire --device-name cpu -tb"
    CMD="python3 $LARMATCH_DIR/deploy_larmatchme_v2.py --config-file ${CONFIG_FILE} --input-larcv $baseinputmod --input-larlite ${baseinput} --weights ${WEIGHTS_DIR}/${WEIGHT_FILE} --output $lm_outfile --min-score 0.3 --adc-name wire --device-name cpu"    
    echo $CMD >> ${local_logfile}
    $CMD >> ${local_logfile} 2>&1

    CMD="python3 $RECO_TEST_DIR/run_kpsrecoman.py --input-dlmerged ${baseinputmod} --input-larflow ${lm_outfile} --output ${reco_outfile} -tb -mc --products min"
    echo $CMD >> ${local_logfile}
    $CMD >> ${local_logfile}
